...
export default function CheckoutForm() {
  ...
  const [cart] = useState([
    { id: 3, name: 'Takiso Web Monthly', price: 150.0, quantity: 1 },
  ]);

  const validCoupons = ['TAKISOWEB35'];
    ...
    <form onSubmit={handleSubmit} className="max-w-lg mx-auto my-8 space-y-6">
        ...
        <div key={field} className="relative">
          ...
          <label
            className={`absolute left-0 transition-all duration-300 pointer-events-none ${
              contact[field] ? '-top-2 text-blue-500 text-sm' : 'top-3 text-gray-400 text-base'
      ...
      <div className="relative">
        <input
          ...
          required
          placeholder=" "
          className={`w-full border-b-2 px-0 py-3 text-base focus:outline-none peer ${
            emailValid ? 'border-gray-300 focus:border-blue-500' : 'border-red-500 focus:border-red-500'
          }`}
        />
        <label
          className={`absolute left-0 transition-all duration-300 pointer-events-none ${
            contact.email ? '-top-2 text-blue-500 text-sm' : 'top-3 text-gray-400 text-base'
          } peer-focus:-top-2 peer-focus:text-blue-500 peer-focus:text-sm`}
        >
          Email *
        </label>
